"""
Pre-Omicron model wrapper.

This module implements the public interface:

    - predict_horizon_pre(country_iso3, week_id_or_idx, policy_sliders)
    - compare_horizons_pre(country_iso3, week_id_or_idx, scenario_sliders)
...
but uses local artifacts under backend/artifacts/pre_omicron and the
...
DATA_PATH = Path(os.getenv("PRE_OMICRON_DATA_PATH", str(_DEFAULT_DATA_PATH)))

if not DATA_PATH.is_file():
    raise FileNotFoundError(
        f"Pre-Omicron history CSV not found at {DATA_PATH}. "
        f"Set PRE_OMICRON_DATA_PATH or place df_final_pre_omicron.csv in backend/data/."
    )
...
def _build_future_policy_windows(
    ...
    """
    g = (
        history_df[history_df["week_id"] <= int(start_week_id)]
        .copy()
        .sort_values("week_id")
    )
    if g.empty:
...
def _build_future_policy_lags(
    ...
    # Build a time axis starting at t0+1
    out = {
...
def compare_horizons_pre(
    ...
    preds_baseline = predict_horizon_pre(country_iso3, week_id, baseline_path)
    preds_scenario = predict_horizon_pre(
